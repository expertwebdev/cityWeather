...
const stateName = document.querySelector('.area_state');
...
myWeather('Bengaluru','Karnataka', 12.9569, 77.7011, _key)
...
async function setCity(city, key) {
    ...
    let geoData = {
        ...
        _lat: response[0].lat,
        _lon: response[0].lon,
        _state: response[0].state
    }
    return geoData;
}

async function myWeather(name, state, lat, lon, key) {
...
function searchWeather() {
    if (searchBar.value) {
        ...
        .then(data => {
            const { name, _lat, _lon, _state } = data;
            myWeather(name, _state, _lat, _lon, _key)
